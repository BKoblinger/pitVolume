...
function [lError, InfoHeader, Map, sequenceLabel]=ReadZMPFile(filename, varargin)
% declarations/defines
sequenceLabel = [];
    optInputArgSize = size(varargin,2);
    if optInputArgSize >= 1        
...
%        message = ferror(fid,'clear');
    ...
    try
        ...
        while ~eof
            ...
            if openRaw == 1 && strncmp(tag, 'Raw', 3);
                tag = tag(4:end);
            end
            ...
            switch tag
                ...
                case 'Sequence'
                    fread(fid,4,'*uint32'); % I have yet to figure out what these 4*32 bits are
                    length = fread(fid,1,'*uint8');
                    sequenceLabel = fread(fid,length,'*char')';
                case 'Data'
        ...
        me=lasterror;
        emessage=strcat('Error reading file : ',filename, ' . ', me.message);
        errordlg(emessage,'File Error','modal');
        lError=-1;
    end
